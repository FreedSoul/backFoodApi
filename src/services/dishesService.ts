...
class dishesService {
    ...
    public async findOne(id: string) {
        ...
        const res:QueryResult = await dbConnect.query('SELECT * FROM dishes WHERE dish_id = $1', [Number.parseInt(id)])
    ...
    public async create(queryFields:Array<string>,valuesField:Array<string|number|boolean|Array<string>>) {
        const dbConnect:Pool = await getConection()
        let auxField:Array<string> = valuesField.map((x, index) => { return `$${index+1}`})
        let values = auxField.join(", ")
        let fields = queryFields.join(", ")
        const queryJoined = `INSERT INTO dishes (${fields}) VALUES (${values}) `
    ...
    public async update(queryFields:Array<string>,valuesField:Array<string|number|boolean|Array<string>>) {
        const dbConnect:Pool = await getConection()
        let fields = queryFields.join(", ")
        const queryJoined:string = `UPDATE dishes SET ${fields} WHERE dish_id = $${valuesField.length} RETURNING *;`
    ...
    public async delete(id: string) {
        const dbConnect:Pool = await getConection()
        const res:QueryResult = await dbConnect.query('DELETE FROM dishes WHERE dish_id = $1 RETURNING *', [Number.parseInt(id)])
        console.log(res)
        return res.rowCount
    }
}
